...
\abstract{We present cosmological parameter constraints estimated
  using the Bayesian \BP\ analysis framework. This method supports
  seamless end-to-end error propagation from raw time-ordered data onto
  final cosmological parameters. As a first demonstration of the
  method, we analyzed time-ordered \Planck\ LFI observations, combined
  with selected external data (\WMAP\ 33--61\,GHz, \Planck\ HFI DR4 353
  and 857\,GHz, and Haslam 408\,MHz) in the form of pixelized maps that are 
  used to break critical astrophysical degeneracies.  Overall, all the results are
  ...
  parameter amounting about $1\,\sigma$ when considering only temperature
  ...
  derived from the joint analysis of official LFI and
  \WMAP\ products. Most importantly, however, we find that the
  uncertainty derived in the \BP\ processing is about 30\,\% greater
  than when analyzing the official products, after taking into account
  the different sky coverage. We argue that this uncertainty is due to
  a marginalization over a more complete model of instrumental and
  astrophysical parameters, which results in more reliable and
  more rigorously defined uncertainties. We find that about 2000 Monte
  Carlo samples are required to achieve a robust convergence for a
  low-resolution cosmic microwave background (CMB) covariance matrix with 225 independent modes, and
...
Universe. The first discovery of these fluctuations is attributed to 
\citet{smoot:1992} and over the past three decades, massive efforts
have been undertaken to produce detailed maps with steadily increasing
...
posits the universe was created during a hot Big Bang event taking place about 13.8
billion years ago; along with the postulate that it was seeded by Gaussian random density
...
terms of the theoretical understanding. Indeed, some of the biggest
questions in modern cosmology revolve around our understanding of the
physical nature of inflation, dark matter, and dark energy, with
enormous sums of money spent on attempts to answer these questions.  In all these studies, CMB
observations play a key role.
...
between 2009 and 2013. These measurements have imposed strong constraints
...
collecting data in 2013, the final official \Planck\ data
release took place as recently as 2020 \citep{npipe}, clearly testifying to the significant analytical challenges associated with these types of data. Large-scale polarization
reconstruction represents a particularly difficult problem, and
massive efforts have been undertaken in the aim to control all
...
nanokelvin on degree angular scales. Detecting such a faint signal
requires at least one or two orders of magnitude higher sensitivity
than \Planck, as well as correspondingly more stringent systematics
...
component separation. Because any CMB satellite experiment 
must, in practice, be calibrated with in-flight observations of astrophysical
sources, the calibration is limited by our knowledge of
...
internal degeneracies. We refer to  \citet{bp01,bp11} for further discussion.
...
lower signal-to-noise ratios (S/N) than the \Planck\ HFI observations. The
cosmological parameter constraints derived in the following are
therefore not in and of themselves competitive in terms of absolute
...
to systematic uncertainties, we adopted the reionization optical depth $\tau$ as a
particularly important probe of stability and performance of the
\BP\ framework and we aimed to estimate $P(\tau\mid\mathbf{d})$ from
\Planck\ \lfi\ and \WMAP\ observations. We also constrained a basic
six-parameter \LCDM\ model, combining the \BP\ low-$\ell$ likelihood
with a high-$\ell$ Blackwell-Rao CMB temperature likelihood that, for
the first time, covers the two first acoustic peaks, or
$\ell\le600$. Eventually, we also complement this with the
...
Eq.~\eqref{eq:data_model}, we include a set of hyperparameters for each
...
evaluated using a standard cosmological Boltzmann solver, as implemented, for instance, 
...
\subsection{\BP\ posterior distribution and Gibbs sampler}
...
normalization factor that is typically referred as the ``evidence.'' Since the
...
we employed Gibbs sampling; that is, rather than drawing samples
directly from the joint posterior distribution, $P(\omega
\mid\mathbf{d})$, we drew samples iteratively from all respective
...
\eqref{eq:cmbcl}, and we refer to \citet{bp01} and
...
matrix given by $\S(C_{\ell})^{-1}+\N^{-1}$, where $\S(C_{\ell})$ and $\N$  are, thus, the
...
astrophysical foreground component, and the $\eta_i$ results  are independent
...
algorithm and, in practice, we introduced a few important modifications
...
prevent these errors from contaminating the CMB power spectrum and
cosmological parameters, we therefore applied a fairly large confidence
...
reason, we ran the analysis in two main stages; we first ran the above
algorithm without a Galactic mask and setting
...
Then, once this main sampling process is done, we resampled the
...
again \citep[see][for full details]{bp11}. For the low-resolution CMB
...
to obtain more samples of the same type as above and to reduce the Monte
Carlo uncertainty in the noise covariance matrix
\citep{sellentin2016}. In this case, we  simply drew $n$
additional samples from Eq.~\eqref{eq:cmbamp}, fixing both the
instrumental and astrophysical parameters, as well as the CMB $a_{\ell
  m}$'s for $\ell>64$. We effectively mapped out the local
conditional distribution with respect to white noise for each main sample
on large angular scales. We conservatively drew $n=50$ new samples per
main sample in this step, but after the analysis started, we
checked that a set of as few as ten~subsamples achieves an equivalent
effect. On the other hand, since the cost of producing one of these
subsamples is almost two orders of magnitude smaller than producing a
full sample, this additional cost is negligible.

This approach is not suitable for a high-resolution CMB temperature
analysis, since we cannot construct a pixel-pixel covariance matrix
with millions of pixels. In this case, we  used the Gaussianized
Blackwell-Rao estimator instead \citep{chu2005,rudjord:2009}, which was also
used for CMB temperature analysis up to $\ell\le30$ by
\Planck\ \citep[e.g.,][]{planck2016-l05}. This estimator relies on
proper $C_{\ell}$ samples and we therefore resampled the main chains
once again, but this time we applied the confidence mask and enable the
...
regime above $\ell\gtrapprox 800$. For this reason, we conservatively limited the current
\BP\ temperature analysis to $\ell\le 600$, leaving some buffer, and
combined it with \Planck\ 2018 results at higher multipoles when
necessary.


\subsection{\BP\ CMB likelihood}
...
\citep[e.g.,][]{page2007,planck2016-l05} and a Blackwell-Rao (BR)
...
that we employed the signal-to-noise eigenmode compression technique
...
thus, the number of Gibbs samples required for convergence); in addition, we were thus able to use the BR estimator to $\ell\le 600$, not
...
approximations and we refer  to the aforementioned papers for further details.
...
a multivariate Gaussian likelihood is expressed as:
\begin{equation}
  \label{eq:gauss_like}
...
The main challenge associated with this approach is related to how many
samples are required for $\N$ to actually converge. As discussed by
\citet{sellentin2016}, a general requirement is for
...
In practice, we compressed the information using the methodology
...
$\ell_\mathrm{t}$,\footnote{Note that these modes do have some sensitivity to higher multipoles due to non-orthogonality of the spherical harmonics on a masked sky; the quoted truncation limit is therefore only approximate.} and with a relative S/N higher than $\epsilon$. It is important to note
...
number of modes is one of the key results presented in
...
\begin{figure*}[t]
        \center
        \includegraphics[width=0.75\linewidth]{BP10_comparison_6params.pdf}
  \caption{Constraints on the six $\Lambda$CDM parameters from the
    \BP\ likelihood (blue contours) using the low-$\ell$ brute-force
...
\begin{table}[t]
  ...
  \newdimen\tblskip \tblskip=5pt
  \caption{
    Constraints on the six \LCDM\ base parameters with confidence intervals at $68\%$ 
...
For the high-$\ell$ temperature analysis, we exploited the Blackwell-Rao
(BR) estimator \citep{chu2005}, which has been demonstrated to work
very well for high S/N data \citep{eriksen:2004}. This is
the case for the \BP\ temperature power spectrum below $\ell \lesssim
700$, whereas the S/N for high-$\ell$ polarization is
very low everywhere, even when combining \lfi\ and \WMAP\ data.

In practice, we employed the Gaussianized Blackwell-Rao estimator (GBR),
...
and systematic uncertainties, it is strictly speaking an approximation; however, as shown by \citet{rudjord:2009}, it is an excellent approximation
...
likelihood. In this paper, we derived $\Lambda$CDM cosmological
...
\begin{figure}[t]
        \center
        \includegraphics[width=\linewidth]{BP10_LCDM_30-600.pdf}
...
The final cosmological parameters were sampled with \texttt{CosmoMC}
...
\citep{neal2005,lewis2013b}. In our analysis, we ran eight chains
...
prior boundary (such as the tensor-to-scalar ratio, $r$), in the
case of which we report upper 95\,\% confidence limits. 

\section{Six-parameter \LCDM\ constraints}
\label{sec:LCDM_constraints}

At this point, we are ready to present standard $\Lambda$CDM cosmological
parameter constraints as derived from the \BP\ likelihood and we
compare them with previous estimates from \Planck\ 2018
...
\BP\ alone, using only the temperature information up to $\ell\le600$ and
...
acoustic oscillations (BAO). We refer to \citet{planck2013-p11} and
\citet{planck2014-a15} for corresponding \Planck\ analyses. 
...
a trivial procedure, as it typically requires joint simulations of \Planck\ 
LFI and HFI observations (in addition to any necessary external dataset) that are 
...
the joint likelihood. Around $\ell = 600$, \BP\ $C_\ell$ values show a 
correlation level of few percent between the nearest and next-to-nearest 
...
and to be be only weakly correlated between the two likelihoods. Finally, 
...
\begin{figure}[ht]
        \center
        \includegraphics[width=\linewidth]{BP10_ext_H0_OmM.pdf} \\
        \includegraphics[width=\linewidth]{BP10_ext_s8_OmM.pdf}
        \caption{2D marginal posterior distributions for the parameter pairs 
...
both differ by about $1\,\sigma$ between \BP\ and \Planck\ 2018 and
...
$30\le\ell\le 600$, and therefore we should, in principle, expect
a perfect agreement between these distributions and that is obviously
...
variations are not individually large enough to move the main
...
\begin{figure*}[t]
  \center
  \includegraphics[width=0.49\linewidth]{BP10_tau_post.pdf} 
  \includegraphics[width=0.49\linewidth]{BP10_r_post.pdf}  
  \caption{Comparison of marginal posterior
    distributions of the reionization optical depth from \Planck\ 2018, shown on the left
    (\emph{red, dotted}; \citealp{planck2016-l06}), 9-year
    \WMAP\ (\emph{green, dot-dashed}; \citealp{hinshaw2012}),
    ...
    amplitude $A_{\mathrm s}$ (\emph{black}).      Corresponding marginal \BP\ tensor-to-scalar ratio posteriors
    ...
    best-fit values, shown on the right. (\emph{black}). The filled region corresponds to
...
angular range. When used in combination with high-$\ell$ \Planck\ 2018
...
uncertainty. The net discrepancy with respect to \citet{Riess2018a} is, thus, effectively unchanged.
...
\begin{figure}[t]
        \center
        \includegraphics[width=\linewidth]{lowl_fsky_tau_r_PTE_BI200_regP05.pdf}
...
As discussed by \citet{bp01}, the main purpose of the \BP\ project has
not been to derive new state-of-the-art $\Lambda$CDM parameter constraints,
for which (as we explain above) \Planck\ HFI data are
...
overall probe of the efficiency of the entire framework. At this point, we are
finally ready to present the main results regarding this parameter, as given below.
...
alone (black curve) and compare this result with the corresponding
...
strongly correlated parameters, that is, $A_{\mathrm{s}}$ and/or
$r$. Taking into account the fact that \citet{natale:2020} analyzed
the official LFI and \WMAP\ products jointly, we chose to tune our
analysis configuration to their findings to facilitate a head-to-head
comparison for the most relevant case. The corresponding numerical values
...
LFI+\WMAP\ value and it is therefore in slightly better agreement with the
HFI-dominated results. Second, and more importantly, we see that the
\BP\ uncertainty is greater for \BP\ than LFI+\WMAP,\, despite
...
smaller. We interpret our greater uncertainty as being due to marginalizing over a more
...
\begin{figure}[t]
        \center
        \includegraphics[width=\linewidth]{BP10_tau_syst_assess.pdf}
...
fraction of \cite{natale:2020}, leading to a constraint of $r<0.94$, which is 
...
somewhat above 68\,\%. For this reason, we conservatively adopted a sky
fraction of 68\,\% for our main results, but we note that 73\,\% would
have been equally well justified. 

Before concluding this section, we return to the importance of
end-to-end error propagation,  performing a simple analysis in which
...
parameters are uncertain and we marginalize over all of them, as in the
...
\begin{figure}[t]
  \center
  \includegraphics[width=\linewidth]{BP10_lowl_tau_R18_Tbp10_burnin200s_regP05.pdf}
...
\begin{figure}[t]
  \center
  \includegraphics[width=\linewidth]{sigmal_GR.pdf}
...
parameters by Gibbs sampling. This permits us to
answer this question quantitatively, using the results 
presented above.

Starting with the low-$\ell$ polarization likelihood, we once again
adopted $\tau$ as a proxy for overall stability, which we show in
Fig.~\ref{fig:convergence} $\tau$ as a function of the number of Gibbs
...
inputs in Eq.~\ref{eq:postmean}.\footnote{Recall that for each main
  Gibbs chain sample, we additionally draw $n=50$ subsamples to
...
derived by comparing the covariance of this sky map with the predicted noise covariance as given by $\N$; any excess fluctuations in
...
described by a greater number of samples.
...
is almost nine times greater than the number of modes in the covariance
...
Bayesian framework for a global analysis of CMB and related datasets
that allows for a joint analysis of both astrophysical and instrumental
effects and, thus, a robust end-to-end error propagation. In this
...
highly sensitive HFI measurements.
...
account the different sky fraction. We argue that this is
...
end-to-end samples are need to achieve robust results. Clearly,
