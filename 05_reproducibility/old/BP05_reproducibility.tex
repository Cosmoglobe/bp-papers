...
% To us, the results from this survey strongly suggest that
% reproducibility is indeed an important component of many CMB
% scientists' day-to-day work -- but there is ample room for improving
% the workflow that is required to support this. 
...
\begin{figure*}[t]
    ...
    \caption{Licensing diagram
      ...
      and the information provided on the \url{https://tldrlegal.com/}.}%{https://tldrlegal.com/}.} 
...
%However, Ninja files are supposed to be produced by the high-level build system such as \texttt{CMake} or 
%Meson\footnote{The full list of software which supports Ninja can be found here: 
%\href{https://github.com/ninja-build/ninja/wiki/List-of-generators-producing-ninja-build-files}
%{https://github.com/ninja-build/ninja/wiki/List-of-generators-producing-ninja-build-files}} which 
%makes it unsuitable to be our new standalone build system.
...
%In \texttt{CMake}, there are two common ways to organize the
%\texttt{CMakeLists.txt} configuration files. The first option is to
%store these within the sub-directories that contain the relevant
%source codes. The main advantage of this is that information is stored
%locally, and it is not necessary to coordinate configuration files
%across the full project, but only locally for each component. {\red
%  (HKE: Is this true and/or relevant? If not, replace with a proper
%  explanation.)} The second common option is to store all
%configuration files in a dedicated \texttt{cmake} folder, which then
%controls compilation of the entire project. This has three main
%advantages. First, it makes the rest of the directory structure
%cleaner and easier to maintain and expand. Second, when developing the
%\texttt{CMake} scripts, it is convenient to find all \texttt{CMake}
%related information in one place. Third, and perhaps most importantly,
%this also allows us to use the so-called \texttt{CMake}
%\texttt{Superbuild} pattern which permits a complete compilation of a
%project and all its dependencies with just a single command. This
%process is completely automated and requires only a handful of
%commands each of which supports a variety of variables to assure fine
%grained control over the installation {\red [footnote to
%    documentation]}. 


%With this approach each dependency is formally
%treated as a separate entity, but \texttt{CMake} is able to
%automatically determine the correct sequence of
%compilation/installation, and passes the details from one project to
%another. For these reasons, we have adopted the single-folder
%\texttt{cmake} approach in \commander.
...
\begin{figure*}[t]
      ...
      even though \commander\ does not require them explicitly. %This is an example 
      %of the so-called ``dependency hell'' when one library is interdependent with 
      %many others hence creating the dependency-cascade.
      }
        %{\red (HKE: Can this figure be made more useful? More
        %information? Other information? Something? Right now, it
        %doesn't really serve a very useful role for the main discussion.)}}
...
%\textbf{Test:} Run the tests provided by the subproject developer given that the test stage is enabled by user [Maksym: not implemented yet!];
...
%using the \texttt{bp} data downloader 
%described in Sect.~\ref{sec:downloader}:
%\begin{verbatim}
%> bp download all
%\end{verbatim}
%========================================================
...
%\section{Documentation, training, and community building}
%\label{sec:documentation}



% The documentation is divided into three main sections. The first section
% considers \href{https://docs.beyondplanck.science/#/parameters/intro}{the
% \commander\ documentation}. Here the user can find a detailed description of the
% input parameters related to a \commander\ run. Each parameter is given a brief
% description, along with the allowed and suggested values. Additionally, the various
% \commander\ input files and data types are described in detail. This section also
% gives a detailed guide on how to install \commander for all use-cases.

% The next documentation section considers
% \href{https://docs.beyondplanck.science/#/pipeline/README}{the \BP\ pipeline}.
% Here the various stages of the pipeline are outlined. [\textbf{TODO}:
% \emph{Find more content to include (documentation is not yet written)}]


% The final documentation section considers how
% \href{https://docs.beyondplanck.science/#/repro/README}{the reproducibility} has
% been implemented in \BP.[\textbf{TODO}: \emph{Find more content to include
% (documentation is not yet written)}]

% [\textbf{TODO}: \emph{Perhaps include an image of what the documentation page looks like?}]


% Describe documentation efforts, with link to web pages. Describe general
% structure. Emphasize dynamic nature; this is never finishing, but will
% continually evolve as more experiments and features are added. Community-based
% science.

% \begin{itemize}
%     \item How reproducibility has been implemented in \BP\
%         \url{https://docs.beyondplanck.science/#/repro/README}
%     \item Overview of \BP\ pipeline
%         \url{https://docs.beyondplanck.science/#/pipeline/README}
%     \item Overview of \commander\ as used in \BP\
%         \url{https://docs.beyondplanck.science/#/parameters/intro}
% \end{itemize}


%\subsection{Discussion forum}
%\BP\ comes with \href{https://forums.beyondplanck.science/}{a discussion web
%forum}. The discussion forum is the first point of contact for help regarding
%any technical questions users might have related to the code, but also for more
%general scientific questions regarding the project. The goal is that the
%discussion forum will act as a database of Frequently Asked Questions once it
%has accumulated some questions. [\textbf{TODO}: \emph{Perhaps include an image of the forum?}]


% Describe web forum, with links. First point of contact for help regarding both
% technical code aspects and science questions. Open up the possibility of joint
% community-driven publications with or without closed channels? 

% \begin{itemize}
%     \item Forum is at \url{https://forums.beyondplanck.science/}. ``The goal
%         with the discussion forum is essentially to build a database of
%         Frequently Asked Questions, so we don't have to answer each single user
%         directly.''
% \end{itemize}

%\subsection{Community-based paper writing} 
