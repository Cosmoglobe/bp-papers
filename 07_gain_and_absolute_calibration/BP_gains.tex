...
\begin{figure}[t]
  \center
%  \includegraphics[width=0.95\linewidth]{figs/dip_quadr_degeneracy.pdf}
...
%In the following, we adopt a similar strategy as \Planck\ 2018, but
%with an important difference: Rather than excluding the
%entire CMB polarization component from the calibration procedure, we
%only exclude the single CMB polarization quadrupole mode. This ensures
%that all higher-order CMB moments are formally estimated without any
%effective transfer function, while the quadrupole formally still does
%have a non-zero transfer function. However, given the high LFI noise
%level and the low $\Lambda$CDM prediction this is in practice
%negligible, as already demonstrated by previous \Planck\ DPC
%analyses. It is also worth noting that the real CMB temperature
%quadrupole amplitude is significantly lower than the $\Lambda$CDM
%prediction \citep{dOC2004}, and one may therefore expect also the
%polarization quadrupole to be low, as these are correlated. Still, for
%cosmological parameters derived from large-scale \Planck\ polarization
%data, one is well advised to compare estimates with and without the
%CMB polarization quadrupole included, and ensure that these are
%statistically consistent. Of course, this particular mode is also
%associated with a high cosmic variance, and a small bias regarding
%this mode is in practice unlikely to have any measurable effect.
...
%  \caption{Smoothing window size limits for the LFI detectors. All detectors within the same frequency band share the same window size limits.}
...
%    \def!{\kern\signwidth}
% \halign{
%   \hbox to 1cm{\hfil#\hfil}\tabskip 1em&
%   \hfil#\tabskip 1em&
%   \hfil#\hfil\tabskip 0pt\cr
%    \noalign{\doubleline}
%      \omit\hfil\textsc{Detector}\hfil&
%      \omit\hfil$w^{\mathrm{min}}$\hfil&
%      \omit\hfil$w^{\mathrm{max}}$\hfil\cr
...
%\subsubsection{Boxcar smoothing}
%The first and most straightforward smoothing approach is to use the same smoothing technique as employed by \Planck\ 2018 -- namely a moving average, where each gain sample is set to be the average of all samples around it within a certain window size. As we in general want the smoothing to preserve as much actual data as possible, while smoothing away nonphysical statistical fluctuations that arise due to lack of data. In the context of the gain estimation described in this paper, the signal-to-noise ratio is largely dependent on the amplitude of the solar dipole, and we therefore let the window size vary according to these amplitudes, with minimum and maximum window sizes given by the values in Table~\ref{tab:window_size}, so that at dipole maxima the lowest window size is used.
%
%
%
%One potential problem with this approach is that there is a risk of too aggressively smoothing the data, which would mean that physical gain fluctuations would be removed from the final solution. This again can then propagate into the other parts of the Gibbs chain, potentially creating nonphysical artifacts in the results.
%
\begin{figure}
  \center
...
\begin{figure*}[t]
  ...
  \includegraphics[width=\linewidth]{figs/gaincomp_030.pdf}
    ...
    %The light grey lines are the locations of hard-coded gain jumps at which we set hard stops for the smoothing algorithm, detailed in Table~\ref{tab:gain_jumps}.
...
\begin{figure*}[t]
  ...
  \includegraphics[width=\linewidth]{figs/gaincomp_044.pdf}
    ...
    %The last half of PIDs for detectors 26M and 26S were not included in the calibration dataset. 
...
%The prior we implement here is based on the assumption that since gain fluctuations are related to fluctuations in the detector electronics, the power spectrum of these fluctuations should behave like $1/f$ noise. We therefore assume that in Fourier space, the gain fluctuation covariance matrix $\tG$ can be written as
...
%  \begingroup
%  \newdimen\tblskip \tblskip=5pt
%  \caption{Gain fluctuation PSD parameter values used in the production run}
%  \label{tab:gain_psd_params}
%  \nointerlineskip
%  \vskip -3mm
%  \footnotesize
%  \setbox\tablebox=\vbox{
...
%   \hfil#\hfil\tabskip 0pt\cr
%    \noalign{\doubleline}
%      \omit\hfil\textsc{Parameter}\hfil&
%      \omit\hfil\textsc{Value}\hfil\cr
%      \noalign{\vskip 4pt\hrule\vskip 4pt}
%        $\alpha$ & -2.5 \cr
%        $\sigma_0$ & $3\cdot 10^{-4} V^2 / K^2$ \cr
%        $f_{\textrm{knee}}$ & 1 hr$^{-1}$\cr
%      \noalign{\vskip 4pt\hrule\vskip 5pt} } }
%  \endPlancktablewide \endgroup
%\end{table}


%The proper Bayesian way of handling this is to assign a covariance
%matrix to $\delta g\qi$, and then sample jointly from the likelihood
%and prior for $\delta g\qi$, as is done for both the instrumental
%noise and CMB fluctuations \citep{bp01,bp06,bp11}. However, due to
%time constraints, this is left for future work. In this paper, we
%instead adopt a similar approach as the \lfi\ DPC and \npipe\ 
%\citep{planck2016-l02,planck2020-LVII}, and smooth $\delta g\qi$
%using a variable-width smoothing window,
%\begin{equation}
%    \overline{\delta g}_{q,i} = \left(\sum\limits_{q-n_{q, i}}^{q+n_{q,
%      i}}w_{q,i}\right)^{-1} \sum\limits_{q-n_{q, i}}^{q+n_{q, i}}w_{q,i}\delta g_{q,i}
%\end{equation}
%where $w_{q,i}\equiv(\T_i^T\N_i^{-1}\T_i)^{-1/2}$ is the inverse noise
%variance weight of $\delta g\qi$, and $n_{q,i}$ is the width of the
%smoothing window. Also following the LFI DPC, we let $n_{q,i}$ vary in
%time, depending on relative strength of the sky signal
%calibrator. Specifically, we define
%\begin{equation}
%    m\qi = \mathrm{Var}(\s^{\mathrm{sky}}\ti, t\in q)
%\end{equation}
%to be the empirical variance of $\s^{\mathrm{sky}}\ti$ as seen by detector $i$
%in scan $q$, and we define a minimum and maximum width,
%$n^{\mathrm{min}}$ and $n^{\mathrm{min}}$, for each frequency channel,
%as listed in Table~\ref{tab:window_size}. We then interpolate linearly
%between these limits based on the local signal strength,
%\begin{equation}
%  n\qi = n^{\mathrm{min}}_i + \frac{m\qi -
%    m^{\mathrm{low}}}{m^{\mathrm{high}} - m^{\mathrm{low}}} (n^{\mathrm{max}}_i - n^{\mathrm{min}}_i),
%\end{equation}
%where $m^{\mathrm{low}}=1\muK$ and $m^{\mathrm{high}}=5\muK$; these
%values are adopted directly from the \lfi\ DPC.
%
%Finally, we recall that we require $\sum_q \delta g\qi=0$, which is
%imposed by subtracting the mean after smoothing $\delta g\qi$
%as described above. This does result in a statistically slightly
%sub-optimal estimate, as it gives equal weight to all scans, rather
%than assigning inverse noise variance weights as in
%Eq.~(\ref{eq:relative_gain_sampling}). Solving the time-dependent
%system with Lagrange multipliers would be computationally
%prohibitively expensive, as it includes more than 44\,000 individual
%PIDs per detector. We note that the current solution is statistically
%unbiased, even if slightly sub-optimal, and we leave implementation of
%the exact Wiener-filter approach to future work.
...
%Overall, most chains are stable and mix well, but there are some
%cases for which the gain solution is slowly drifting, and do not
%properly converge to a stationary value within the length of the Gibbs
%chain. A particularly bad example is the 30\,GHz LFI 27M radiometer,
%PID 15\,431. These situations are in general more common for the
%30\,GHz channel than the higher frequencies, because of the higher
%foreground signal in this channel. They are also more common for
%PIDs for which the \Planck\ scanning strategy is closely aligned with
%the Galactic plane, since a relatively larger fraction of the data are
%rejected by the processing mask. 

%To globally characterize these issues, Fig.~\ref{fig:corrlengths} shows the correlation lengths across all PIDs. The black line shows the estimated mean correlation length, whereas the blue bands show the estimated standard deviation. Thus, for 44 and 70\,GHz essentially all cases have a correlation length shorter than 10--15 samples, and therefore mix well. However, the correlation lengths for the 30\,GHz channel are slightly worse, especially for the 27S and 28M radiometers, where samples separated by 20--30 steps have a mean correlation of roughly 10\,\%. This behaviour is likely to be improved in future processing, when better foreground modelling will allow a smaller processing mask.
...
%For 44\,GHz there is mostly good agreement between \BP\ and \Planck\ PR4, though certain detectors are not completely spot on. There is also good agreement between \BP\ and the \Planck\ 2018 mod For 70\,GHz the agreement between \BP\ and \Planck\ PR4 is generally
%excellent, while the 2018 model generally has slightly higher absolute
%calibration than other two in the 70\,GHz channel. The main visual
%difference between \BP\ and PR4 lies in the fact that
%\BP\ smooths the gain solution after estimation, while PR4 does
%not. 
...
%Although we are using Wiener filtering to smooth the gain solutions in this paper, we have also experimented with other smoothing methods. Following \Planck 2018 and \Planck\ PR4, we implemented a boxcar moving average, where the gain of each scan would be assigned the average of the base gain solutions in the window around. The window size would vary roughly according to the amplitude of the dipole signal for that scan, as this is a robust predictor of the signal-to-noise ratio.
...
%As noted in Sec. \ref{sec:temporal_var_sampling}, we have used both boxcar averaging and Wiener filtering to smooth the gain solutions.
%
%With the boxcar average smoothing, the gain solutions behaves more like the original \Planck\ 2018 solutions than for the case of Wiener filtering. However,  we found that when using the boxcar smoothing procedure, the correlated noise solutions would end up with strong ``stripes'' in the binned polarization maps, especially Stokes $Q$. Reducing the window sizes would mitigate this, and it therefore is likely that this phenomenon is related to an ``over-smoothing'' of the gain. Although reducing the window sizes improved this situation, it unfortunately introduced other issues, like gain spikes in the low signal-to-noise regime.
...
\label{sec:gainjumps}
As noted from the beginning of the \Planck\ experiment \citep[see, e.g.,][]{planck2013-p05}, the physical gain of the instrument exhibits several sharp jumps. These jumps are related to changes in the thermal environment of the instrument -- an example of such an event is the turning off of the 20\,K sorption cooler. Not all of the events are well understood, and can mainly be traced after an initial gain estimation. %and list the jumps that we have considered in our data analysis in Table \ref{tab:gain_jumps}. %These jumps mainly enter the analysis when we consider the smoothing of time-dependent gains in Sect.~\ref{sec:temporal_var_sampling}.
...
%  \begingroup
%  \newdimen\tblskip \tblskip=5pt
%  \caption{PIDs of gain jumps considered in the \BP\ analysis. We indicate whether a jump comes from the \Planck\ PR4 analysis or whether it comes from finding sharp jumps in the 20\,K onboard temperature sensor readout. }
%  \label{tab:gain_jumps}
%  \nointerlineskip
%  \vskip -3mm
%  \footnotesize
%  \setbox\tablebox=\vbox{
...
%    \def!{\kern\signwidth}
% \halign{
%      \hbox to 1cm{#\leaderfil}\tabskip 1em&
%      \hfil#\hfil\tabskip 0pt\cr
%    \noalign{\doubleline}
%      \omit\hfil\textsc{PID}\hfil&
%      \omit\hfil\textsc{Jump classification}\hfil\cr
%      \noalign{\vskip 4pt\hrule\vskip 4pt}
%         *3352  &PR4 \cr
%         *5030  & PR4 \cr
%         *5484  & PR4 \cr
...
%We mentioned in sec. \ref{sec:gainjumps} that the presence of unphysical sharp jumps in the gain factor must be taken into account when doing any kind of smoothing of the gain solutions, since such smoothing assumes some correlations between the various gain factors, and such jumps represent areas where these assumptions break down. For our intermediate boxcar smoothing analysis, we would explicitly make sure that any smoothing window did not extend past any known gain jumps.
%
%In the Wiener filtering smoothing procedure, however, we found that there was no need to explicitly take these jumps into account, and that the implicit balance between prior information and data automatically made sure these jumps were accounted for.
%
%The \BP\ 44\,GHz correlated noise maps
%exhibit a number of strong ``stripes'' in the 44\,GHz channel. This
%effect is shown in Fig.~\ref{fig:corrstripes} for the Stokes $Q$
%parameter, as reproduced from \citet{bp10}. Significant effort has
%been spent on identifying the origin of these stripes, but no firm
%conclusions have been reached yet. One example of the tests that we
%have done is to reduce the smoothing window widths listed in
%Table~\ref{tab:window_size} by a factor of two. In this case, we do
%note that the stripes disappear from $\n_{\mathrm{corr}}$, but at the
%same time, we also see large spikes appearing in the gain solution for
%PID ranges for which the scanning direction is aligned with the dipole
%minimum, and thus for which the signal-to-noise is low for purposes of gain
%estimation.

%Another line of investigation has been to modify the processing masks
%shown in Fig.~\ref{fig:proc_mask}, noting that strong point sources
%or signal gradients can induce stripes like this. However, we have not
%yet been able to construct any mask that solves this problem, or,
%indeed, modifying the general behaviour of the stripes notably.

%A third hypothesis that has been considered is the potential presence
%of undetected large gain jumps, similar to those listed in
%Table~\ref{tab:gain_jumps}. In the current implementation, this list
%is adopted without modification from previous analysis
%pipelines. Noting that the 44\,GHz radiometers generally appear more
%susceptible to non-thermal artefacts than the other two frequency
%channels, it is reasonable to speculate that some subtle hardware
%failures could affect one or more of the 44\,GHz radiometers, similar
%to what is seen in the 44\,GHz LFI 26S radiometer around PID
%20\,800. We have, however, not yet been able to identify any obvious
%gain jumps of this type, and this is therefore still only speculation.

%Understanding and mitigating these stripes is clearly one of the top
%priorities for future \BP\ analysis, as they currently appear to limit
%our ability to extract reliable CMB polarization information from the
%Southern Galactic hemisphere \citep{bp11,bp12}.
