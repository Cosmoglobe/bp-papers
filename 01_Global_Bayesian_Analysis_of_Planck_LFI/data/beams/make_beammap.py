...
for filename, outfile, coltype, vmin, vmax, dataset, freq, unit, scale, comp, labelpos, labels, bar in [
    (directory+"sidelobe.fits", "sidelobe.pdf", 0, -5., -1, r"", r"", r"dB", 1, 0, [-5,-3,-1], [r"$-50$", r"$-30$", r"$-10$"], 1),
]:
    
    m = N.log10(N.abs(hp.ma(hp.read_map(filename,comp))*scale))
    if coltype == 2:
        ind = N.where(m > 0)
    ...
    for width in [8.8]:
        for cmap, colormaptag in [(None, ''), (colombi1_cmap, "colombi1_")]:
            ...
            if bar :
                ...
                cb.ax.xaxis.set_label_text(unit, size=8)
            ...
            plt.text(0.5,  -0.2, r"Primary spillover", ha='left', va='center', color='white', fontsize=7)
            plt.text(0.5,  0.76, r"Secondary spillover", ha='left', va='center', color='white', fontsize=7)
            plt.text(0.8,  1.1, r"Main beam", ha='left', va='center', color='white', fontsize=7)

            plt.arrow(0.77, 1.1, -0.8, 0.4, color='white', head_width=0.2, head_length=0.2)
            plt.arrow(0.47, 0.76, -0.4, 0.35, color='white', head_width=0.15, head_length=0.15)
            plt.arrow(0.47, -0.2, -0.7, 0.2, color='white', head_width=0.1, head_length=0.1)
